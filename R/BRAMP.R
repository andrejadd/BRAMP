...
##   mcmc_obj      : A MCMC data structure of class 'bramp'. The same that is returned from BRAMP().
##                   If this is passed as argument back to BRAMP _and_ 'nr_iterations' is greater
##                   than the iterations in 'mcmc_obj', the MCMC simulation is continued.
##
## Returns:
##
##   A data structure of class 'bramp' containing the following variables:
##
##    MCMC.chain   : A list containing all records related to the chain samples.
##    Grid.obj     ' A structure containing variables related to the Mondrian process and segmentation. 
##    HYPERvar     : A list of hyper-parameters.
##    edge_probs   : A vector with the edge probabilities extracted from the MCMC chain.
##    mean_coefficients : A vector with the mean regression coefficients extracted from the MCMC chain.
##    X            : The n-by-m design matrix.
##    y            : The m-length response vector.
##
BRAMP <- function(y = NULL, 
    ...
    ## If no 'mcmc_obj' variable was passed, check the input data necessary to
    ##  run a MCMC simulation from scratch.
    if(is.null(mcmc_obj)) {
  ...
  start.budget = 1
    ...
    ## The start of iteration can be changed through loading an already existing 'mcmc_obj'
  start.iter = 1
  ...
  HYPERvar = NULL
    ...
    ##
    ## Check if the 'mcmc_obj' variable was passed to BRAMP().
    ##  If yes, extract the MCMC chain and the data. This also means that no additional data has to
    ##  be considered. Continue running the MCMC chain, if the number of iterations ('nr_iteration')
    ## passed to BRAMP() is larger than the chain in the result file.
    ##
  
    if(!is.null(mcmc_obj)) {
  
        ## Check if the data structure is of class 'bramp'.
        if(class(mcmc_obj) != "bramp") {
            cat("[", method.name, "] Variable 'mcmc_obj' is not of class 'bramp'. Exiting ...\n")
            return(-1)
        }
        ...
        ## 
        MCMC.chain = mcmc_obj$MCMC.chain
        Grid.obj = mcmc_obj$Grid.obj
        HYPERvar = mcmc_obj$HYPERvar
